...
struct cache_ctx {
	...
	fdns_core *core;
	ffmap cache; // { qtype, qname } -> cache_entry*
	ffrbtree timetree;
	ffsize total_size;
	ffuint misses, hits;
};
...
static ffsize cache_entry_size(struct cache_entry *ce)
{
	return sizeof(struct cache_entry) + ce->key.len + ce->data.len;
}

static void cache_entry_free(struct cache_entry *ce)
{
	ffmem_free(ce);
}

static void cache_entry_remove(struct cache_ctx *cx, struct cache_entry *ce, ffuint hash)
{
	ffrbt_rm(&cx->timetree, &ce->expire_node);
	ffmap_rm_hash(&cx->cache, hash, ce);
	cx->total_size -= cache_entry_size(ce);
	cache_entry_free(ce);
}

static int cache_map_keyeq(void *opaque, const void *key, ffsize keylen, void *val)
{
	(void)opaque;
...
static void cache_clear(struct cache_ctx *cx)
{
	...
	ffmap_free(&cx->cache);
	ffmap_init(&cx->cache, cache_map_keyeq);
...
static struct cache_entry* cache_entry_new(const ffdns_question *q, ffstr data, ffuint *new_size)
{
	*new_size = sizeof(struct cache_entry) + 4 + q->name.len + data.len;
	struct cache_entry *ce = ffmem_alloc(*new_size);
...
static void remove_old(struct cache_ctx *cx, ffuint new_size)
{
	...
	while (cx->total_size + new_size >= cx->conf.max_size) {
		...
		ce = FF_STRUCTPTR(struct cache_entry, expire_node, node);
		debug("cache: removing entry %S", &ce->name);
		ffuint hash2 = ffmap_hash(ce->key.ptr, ce->key.len);
		cache_entry_remove(cx, ce, hash2);
	}
}
...
static void cache_add(struct cache_ctx *cx, const struct dns_msg *msg, ffstr data)
{
	...
	ffuint ttl = ffmax(msg->ttl, cx->conf.min_ttl);
	if (msg->h.rcode != FFDNS_NOERROR)
		ttl = cx->conf.error_ttl;
	if (ttl == 0)
		return;

	ffuint new_size;
	struct cache_entry *ce, *ce2;
	ce = cache_entry_new(&msg->q, data, &new_size);
	...
	if (ce2 != NULL) {
		...
		cache_entry_remove(cx, ce2, hash);
		ce2 = NULL;
	}
	...
	fftime expire = fftime_monotonic();
	expire.sec += ttl;
...
static int cache_find(struct cache_ctx *cx, const ffdns_question *q, ffstr *data)
{
	...
	struct cache_entry *ce = ffmap_find_hash(&cx->cache, hash, key.ptr, key.len, NULL);
	ffvec_free(&key);
...
void set_ttl(struct cache_ctx *cx, ffstr resp, ffuint ttl)
{
	ffdns_header h = {};
...
void cache_conf(fdns_core *core, const ffconf_arg **conf_args, void **conf_obj)
{
